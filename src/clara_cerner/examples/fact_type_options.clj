(ns clara-cerner.examples.fact-type-options
...
(defsession empty-session
            [too-cold weather-status]
...
(defn run-examples
  []
  [
   {:description    "Query result with a fact of type :temperature-reading"
    :weather-status (-> empty-session
                        (insert {:weather-fact-type :temperature-reading
                                 :temperature       -10})
                        fire-rules
                        (query weather-status))}
   {:description    "Query result with a custom ancestors function and a fact with a type that descends from :temperature-reading"
    :weather-status (-> empty-session
