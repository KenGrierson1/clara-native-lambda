(ns clara-cerner.examples.insta
...
(defsession compiled-session
            'clara-cerner.examples.insta
            (load-user-rules example-rules))
...
(defn run-examples
  ...
  (let [session (-> compiled-session
    ...
    {:discounts (query session get-discounts)
     :promotions (query session get-promotions)}
    ))
